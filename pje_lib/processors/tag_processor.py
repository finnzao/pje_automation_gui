"""
Processador para download de processos por etiqueta.
"""
...
class TagProcessor(BaseProcessor):
    """
    Processador especializado para downloads por etiqueta.
    
    Implementa:
    - Busca de etiquetas
    - Listagem de processos da etiqueta
    - Download em lotes
    """
    ...
    def processar_generator(
        ...
        relatorio = self._criar_relatorio_base(
            "download_por_etiqueta",
            diretorio,
            0  # Será atualizado após listar processos
        )
        relatorio["etiqueta"] = nome_etiqueta
        ...
        try:
            ...
            # ============ FASE 1: BUSCAR ETIQUETA ============
            self._check_cancelado_raise("busca de etiqueta")
            
            relatorio["status"] = "buscando_etiqueta"
            yield relatorio
            
            etiqueta = self.tag_service.buscar_etiqueta_por_nome(nome_etiqueta)
            ...
            processos = self.tag_service.listar_processos_etiqueta(etiqueta.id)
